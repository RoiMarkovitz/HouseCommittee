...
class WorksDetailsWindow(BaseView):
    ...
    def __init__(self, master):
        ...
        self.get_works_details()

    def get_works_details(self):
        try:
            obj_type = Connection.CONN.gettype("WORKS_TBL_TYPE")
            cur = Connection.CONN.cursor()
            return_obj = cur.callfunc('get_all_works', obj_type)
            self.dict_works_details = ObjectsHandler.ObjectRepr(return_obj)

        except Exception as err:
            print('Exception occurred while executing the func  ', err)
            self.activate_label_error(err)
        else:
            print("get_all_works function executed")
            self.show_all_details()
        finally:
            cur.close()

    def show_all_details(self):
        for index in range(len(self.dict_works_details)):
            records = ['', '', '', '', '']
            for i, key in enumerate(self.dict_works_details[index]):
                if key == 'WORK_NUMBER':
                    records.insert(0, self.dict_works_details[index][key])
                if key == 'WORK_TYPE':
                    records.insert(1, self.dict_works_details[index][key])
                if key == 'PRICE':
                    records.insert(2, self.dict_works_details[index][key])
                if key == 'BUSINESS_NUMBER':
                    records.insert(3, self.dict_works_details[index][key])
                if key == 'TENANT_ID':
                    records.insert(4, self.dict_works_details[index][key])
    ...
    def set_tree_headers(self):
        ...
        """
        header = ['Work #', 'Work Type', 'Price', 'Business #', 'Tenant ID #']
    ...
    def init_widgets(self):
        ...
        topic_label = TopicLabel(topic_frame, text="Work Details", size=50).get_label()
        topic_label.pack()
