...
class ApartmentsHistoryWindow(BaseView):
    ...
    def get_apartments_history_details(self):
        try:
            ...
            print("get_all_apartment_history function executed")
            self.show_all_details()
        finally:
            cur.close()

    def show_all_details(self):
        for index in range(len(self.dict_apartments_history)):
            records = ['', '', '', '', '', '', '']
            for i, key in enumerate(self.dict_apartments_history[index]):
                ...
                if key == 'LAST_NAME':
                    records.insert(5, self.dict_apartments_history[index][key])
                if key == 'PHONE_NUMBER':
                    records.insert(6, self.dict_apartments_history[index][key])
    ...
    def set_tree_headers(self):
        ...
        """
        header = ['Apartment #', 'Start Date', 'Tenant ID #', 'End Date',
        ...
        self.records_tree.column(header[1], anchor=CENTER, width=100)
        self.records_tree.column(header[2], anchor=CENTER, width=100)
        self.records_tree.column(header[3], anchor=CENTER, width=100)
        self.records_tree.column(header[4], anchor=CENTER, width=100)
        self.records_tree.column(header[5], anchor=CENTER, width=100)
        self.records_tree.column(header[6], anchor=CENTER, width=120)
    ...
    def init_widgets(self):
        ...
        topic_label = TopicLabel(topic_frame, text="Apartments History", size=50).get_label()
        topic_label.pack()
