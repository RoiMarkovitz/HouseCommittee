...
class ApartmentsPaymentsMonthlyWindow(BaseView):
    ...
    def add_apartment_monthly_payment(self):
        try:
            cur = Connection.CONN.cursor()
            cur.callproc('add_apartment_monthly_payment',  [DateUtil.date_converter(self.payment_due_date_stringVar.get()),
            ...
            print("add_apartment_monthly_payment procedure executed")
    ...
    def init_widgets(self):
        ...
        topic_label = TopicLabel(topic_frame, text="Add Apartment Monthly Payment", size=40).get_label()
        topic_label.pack()

        # define stringVar for payment_due_date input
        self.payment_due_date_stringVar = StringVar(input_frame, value="Type pay due date DD-MM-YYYY")
        ...
        apartment_number_entry.grid(row=1, column=0, pady=10, columnspan=2)

        # create button to submit input
        button_submit = Button(input_frame, text="Submit", command=self.add_apartment_monthly_payment)
