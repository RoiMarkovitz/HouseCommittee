...
class TenantsElectionsUpdateWindow(BaseView):
    ...
    def remove_from_elections(self):
        try:
            ...
            cur.callproc('remove_from_elections',
                         [DateUtil.date_converter(self.election_date_stringVar),
                          int(self.tenant_id_stringVar) ] )
    ...
    def init_widgets(self):
        ...
        topic_label = TopicLabel(topic_frame, text="pdate Elections", size=40).get_label()
        topic_label.pack()

        # define stringVar for nickname input
        self.election_date_stringVar = StringVar(input_frame, value="Type date DD-M-YYYY")
