...
class TenantsElectionsDetailsWindow(BaseView):
    ...
    def get_elections_details(self):
        try:
            obj_type = Connection.CONN.gettype("ELECTIONS_TBL_TYPE")
            cur = Connection.CONN.cursor()
            return_obj = cur.callfunc('get_all_elections', obj_type)
            self.dict_elections_details = ObjectsHandler.ObjectRepr(return_obj)

        except Exception as err:
            print('Exception occurred while executing the func  ', err)
            self.activate_label_error(err)
        else:
            print("get_all_elections function executed")
            self.show_all_details()
        finally:
            cur.close()

    def show_all_details(self):
        for index in range(len(self.dict_elections_details)):
            records = ['', '', '', '']
            for i, key in enumerate(self.dict_elections_details[index]):
                ...
                if key == 'VOTES':
                    records.insert(2, self.dict_elections_details[index][key])
                if key == 'S_CHOSEN':
                    records.insert(3, self.dict_elections_details[index][key])
    ...
    def set_tree_headers(self):
        ...
        """
        header = ['Elections Date', 'Tenant ID #', 'Votes', 'Chosen']
    ...
    def init_widgets(self):
        ...
        topic_label = TopicLabel(topic_frame, text="Elections Details", size=50).get_label()
        topic_label.pack()
